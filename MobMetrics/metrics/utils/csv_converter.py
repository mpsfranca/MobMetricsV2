"""Utilities for loading node trajectories from a scenario file and exporting them as CSV."""

from django.conf import settings


class Node:
    """Represents a node and the sequence of positions associated with it."""
    ...
    def addPosition(self, nodePosition):
        """Adds a position record to the node."""
        self._positions.append(nodePosition)

    def getPositions(self):
        """Returns the list of recorded positions."""
...
class NodePosition:
    """Represents the position of a node at a given time."""
...
def nodePopulate(scenario_name):
    """Loads node positions from the scenario file into memory."""
    with open(f"{settings.AUX_PATH}/{scenario_name}.csv", "r") as bm:
        for line in bm:
            ...
            current_node_position = NodePosition(
                current_node_time,
                current_node_x,
                current_node_y
            )
...
def csvWrite(name):
    """Writes the loaded node positions to a normalized CSV file."""
    with open(f"{settings.AUX_PATH}/{name}.csv", "w") as fcsv:
        ...
        for key in nodes.keys():
            ...
            for position in positions:
                fcsv.write(
                    f"{position._time},{position._x},{position._y},{nodes[key]._id}\n"
                )


def convert(name):
    """Converts a scenario file into the output CSV representation."""
    nodePopulate(name)
    csvWrite(name)
